import os, uuid, shutil, requests as req
...
@app.get("/health")
def health():
    # Test if we can reach Vizard at all
    try:
        r = req.get("https://elb.vizard.ai", timeout=5)
        vizard_reachable = True
    except Exception as e:
        vizard_reachable = str(e)
    return {"status": "ok", "vizard_reachable": vizard_reachable, "base_url": BASE_URL}
...
async def upload(file: UploadFile = File(...)):
    ...
    try:
        ...
        if data.get("code") == 2000 and pid:
            jobs[job_id] = {"status": "processing", "project_id": str(pid), "clips": []}
        else:

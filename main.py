...
# Configure Cloudinary (add your API secret in .env)
cloudinary.config(
    cloud_name=os.getenv("CLOUDINARY_CLOUD_NAME", "de5jdqth5"),
    api_key=os.getenv("CLOUDINARY_API_KEY", "725389195426886"),
    api_secret=os.getenv("CLOUDINARY_API_SECRET", "pOsrfuxBPy8JmiVfSIbmNz_b6s0"),
    secure=True
)
...
def get_video_clips(
    ...
    for clip in clips.data:
        ...
        if clip.get("title"):
            ...
            transcript = clip.get("transcript", "")
            formatted_clip["title"] = transcript[:50] + ("..." if len(transcript) > 50 else "") if transcript else f"Clip {clip.get('id', '')}"
...
def get_user_details(current_user: dict = Depends(get_current_user)):
    ...
    plan_features = {
        "starter": {"name": "Starter", "max_videos_per_month": 10, "max_clips_per_video": 20, "ai_transcription": True, "export_formats": ["mp4"], "priority_support": False},
        "pro": {"name": "Pro", "max_videos_per_month": 100, "max_clips_per_video": 50, "ai_transcription": True, "export_formats": ["mp4", "srt", "txt"], "priority_support": True, "custom_branding": True},
        "agency": {"name": "Agency", "max_videos_per_month": 1000, "max_clips_per_video": 100, "ai_transcription": True, "export_formats": ["mp4", "srt", "txt", "json"], "priority_support": True, "custom_branding": True, "api_webhooks": True, "team_members": True}
    }
    return {
        "email": current_user["email"],
        "full_name": current_user["full_name"],
...
# ===================== NEW ENDPOINT: UPLOAD VIDEO FILE DIRECTLY =====================
...
async def upload_and_process_video(
    ...
    """
    Upload a video file directly, upload it to Cloudinary, then process with Vizard.
    ...
    try:
        ...
        # Construct the request object for the existing process_video endpoint
        video_request = ProcessVideoRequest(
            video_url=secure_url,
            video_type=1,          # direct URL
            ext=ext,
            lang="auto",
            subtitle_switch=1
        )
        # Reuse the existing processing logic
        return await process_video(video_request, current_user)
